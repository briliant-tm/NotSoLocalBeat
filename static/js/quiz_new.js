...
function showReadyScreen() {
    ...
    const isHost = room && room.players.some(p => p.is_host && p.username === '{{ user.username }}');
    ...
    fetch('/api/question?' + params)
        ...
        .then(data => {
            ...
            if (data.source === 'YOUTUBE') {
                // For YouTube, we need to handle audio differently
                // Using a proxy or download method
                audio.src = `data:audio/wav;base64,UklGRkYAAAAASUNLAQAA`;
                loadYouTubeAudio(data.youtube_url, data.start_time);
            } else {
...
function loadYouTubeAudio(url, startTime) {
    // This is a limitation: YouTube videos cannot be directly streamed
    // In a real implementation, you would need a backend service to handle this
    // or use YouTube's official iframe API
    console.log('YouTube URL:', url);
    // For now, we'll fall back to a placeholder
    setTimeout(() => {
        isPlaying = true;
        replayBtn.disabled = true;
        replayBtn.innerText = "YOUTUBE MODE";
    }, 1000);
}
...
function submit(ans, uiElement) {
    ...
    if (nextData.source === 'YOUTUBE') {
        submitData.correct_title = nextData.title;
    }
